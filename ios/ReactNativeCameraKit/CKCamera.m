...
static void * CapturingPhotoImageContext = &CapturingPhotoImageContext;
...
- (void) setupCaptureSession {
    ...
    dispatch_async( self.sessionQueue, ^{
        ...
        // Camera for iphone 15 start
        AVCapturePhotoOutput *photoImageOutput = [[AVCapturePhotoOutput alloc] init];
        NSLog(@"PHOTO IMAGE INIT: %@", photoImageOutput);
        if ([self.session canAddOutput:photoImageOutput]) {
            NSLog(@"Adding Photo Image to Output: %s", ">>>>>>>>>>>");
            [self.session addOutput:photoImageOutput];
            self.photoImageOutput = photoImageOutput;
        } else {
            self.setupResult = CKSetupResultSessionConfigurationFailed;
        }
...
- (void)captureOutput:(AVCapturePhotoOutput *)output didFinishProcessingPhoto:(AVCapturePhoto *)photo error:(NSError *)error {
    NSLog(@"CAPTURE OUTPUT: %s", "Method Invoked");
    if (error) {
        NSLog(@"Error capturing photo: %@", error);
        // Handle the error appropriately
    } else {
        NSLog(@"Capturing photo: %@", photo);
        NSLog(@"Capturing photo file: %@", [photo fileDataRepresentation]);
...
- (void)snapStillImage:(NSDictionary*)options success:(CaptureBlock)onSuccess onError:(void (^)(NSString*))onError {
    ...
    NSLog(@"CAMERA 15 DEBUG: %s", "snapping still image");
    NSLog(@"PHOTO IMAGE: %@", self.photoImageOutput);
    AVCapturePhotoSettings *settings = [AVCapturePhotoSettings photoSettingsWithFormat:@{AVVideoCodecKey: AVVideoCodecTypeJPEG}];
    self.semaphore = dispatch_semaphore_create(0);
    [self.photoImageOutput capturePhotoWithSettings:settings delegate:self];
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    NSLog(@"Captured Image Data: %@", self.capturedImageData);
    [self writeCapturedImageData:self.capturedImageData onSuccess:onSuccess onError:onError];
    [self resetFocus];
}
...
- (void)writeCapturedImageData:(NSData *)imageData onSuccess:(CaptureBlock)onSuccess onError:(void (^)(NSString*))onError {
    NSLog(@"WRITE CAPTURE IMAGE: %@", imageData);
...
- (void)addObservers
{
    ...
    if (!self.isAddedOberver) {
        ...
        [self.photoImageOutput addObserver:self forKeyPath:@"capturingPhotoImage" options:NSKeyValueObservingOptionNew context:CapturingPhotoImageContext];
...
- (void)removeObservers
{
    if (self.isAddedOberver) {
        ...
        [self.photoImageOutput removeObserver:self forKeyPath:@"capturingPhotoImage" context:CapturingPhotoImageContext];
